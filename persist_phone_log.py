# persist_phone_log.py — GitHub CSV persistence for TDM Service Desk
...
GH_OWNER   = _SECRETS.get("GH_OWNER", "DumbellMan")     # UPDATE THIS TO YOUR ORG/ACCOUNT
GH_REPO    = _SECRETS.get("GH_REPO", "TDM-Service-Desk")  # TDM repo instead of BearPath
...
def load_phone_log() -> pd.DataFrame:
    """Load phone log CSV from GitHub; return empty DF if not found."""
...
def save_phone_log(df: pd.DataFrame, message: str = "Update phone log"):
    """Overwrite CSV in GitHub with full DataFrame."""
    if df is None:
...
def append_phone_log(row: dict, message: str = "Add phone log entry"):
    """Append a single row by loading → concat → saving."""
    df = load_phone_log()

    df = pd.concat(
        [df, pd.DataFrame([{k: str(row.get(k, "")) for k in PHONE_COLS}])],
        ignore_index=True,
    )
